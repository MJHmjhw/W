...
<!-- 				</select> -->
					<form action="not" id="qw">
						<input type="hidden" name="q" value="selectF"> <input
							type="text" name="name"> <input type="submit" value="提交">
					</form> <a href="not?q=addN"><button id="a12" class="btn btn-primary">发布公告</button></a>
				</td>
			</tr>
		</table>
		<table border="1px " class="table table-bordered table table-hover">
			...
			<c:forEach items="${list}" var="notice">
				<tr>
					...
					<td><a class="bu1"
						href="not?q=selectById&notice_id=${notice.notice_id}"><i
							class="fa fa-pencil"></i></a> <a class="bu2"
						href="not?q=deleteNotice&notice_id=${notice.notice_id}"><i
	...
	<div id="pa">
		...
		<ul class="pagination">
			<li><c:if test="${page2>1}">
					<a href="not?q=allNotice&page=${page2-1}">上一页</a>
				</c:if></li>
			<li class="active">
			...
			<li><c:if test="${page2<=page}">
					<a href="not?q=allNotice&page=${page2+1}">下一页</a>
				</c:if></li>
		</ul>
		</nav>
	</div>
