				...
				<tr>
					...
					<td><c:if test="${msg.sender==user.user_id}">
							<c:forEach items="${list1}" var="all">
								<c:if test="${msg.receiver==all.user_id }">
										${all.username}
									</c:if>
							</c:forEach>
						</c:if></td>
					...
					<td>
					<a href="msg?ms=updateCG&message_id=${msg.message_id}"><i class="fa fa-trash-o" /></i></a>
					<a href="msg?ms=deleteCG&message_id=${msg.message_id}"><i class="fa fa-times"></i></a>
					<a href="msg?ms=selectById&message_id=${msg.message_id}"><i class="fa fa-pencil"></i></a> 
					</td>
				</tr>
			</c:forEach>
		...
		<div id="pa">
			...
			<ul class="pagination">
				...
				<li><c:if test="${page2>1&&page2<=page}">
						<a href="msg?n=sendAll&page=${page2-1}">上一页</a>
					</c:if></li>
				<li class="active"><a>${page2}</a></li>

				<li><c:if test="${page2>=1&&page2<page-1}">
						<a href="msg?n=sendAll&page=${page2+1}">下一页</a>
					</c:if></li>
